...
def init_ui_assets():
    ...
    _ui_assets["winner_image"] = pygame.transform.scale(pygame.image.load("assets/winner.png"), (300, 150))
    _ui_assets["go_image"] = pygame.transform.scale(pygame.image.load("assets/go.png"), (300, 150))
    _ui_assets["pupuk_icon"] = pygame.image.load("assets/pupuk.png").convert_alpha()
    _ui_assets["penyiram_icon"] = pygame.image.load("assets/siram.png").convert_alpha()
    _ui_assets["bom_icon"] = pygame.image.load("assets/bom.png").convert_alpha()

def get_font(name):
    if name == "font_pixel_small":
        return pygame.font.Font("assets/fonts/pressstart2p.ttf", 16)
...
def draw_game(screen, game_instance):
    ...
    icon_size = (80, 80)
    spacing = 60
    total_width = icon_size[0] * 3 + spacing * 2
    start_x = (screen_width - total_width) // 2
    y_pos = 520
    powerup_buttons = []
    icons = [_ui_assets["pupuk_icon"], _ui_assets["penyiram_icon"], _ui_assets["bom_icon"]]
    mouse_x, mouse_y = pygame.mouse.get_pos()

    for i, icon in enumerate(icons):
        x = start_x + i * (icon_size[0] + spacing)
        rect = pygame.Rect(x, y_pos, *icon_size)
        is_hovered = rect.collidepoint(mouse_x, mouse_y)
        scale = 1.2 if is_hovered else 1.0
        scaled_width = int(icon_size[0] * scale)
        scaled_height = int(icon_size[1] * scale)
        ...
        screen.blit(scaled_icon, (scaled_x, scaled_y))
...
def draw_game_won(screen):
    ...
    screen_width, screen_height = screen.get_size()
    win_rect = _ui_assets["winner_image"].get_rect(center=(screen_width // 2, screen_height // 2 - 80))
    screen.blit(_ui_assets["winner_image"], win_rect)
    font_pixel = _ui_assets["font_pixel_small"]
    message_lines = ["you did it! you're goated!", "but can you do it again?"]
    for i, line in enumerate(message_lines):
        text_surface = font_pixel.render(line, True, (255, 255, 255))
        text_rect = text_surface.get_rect(center=(screen_width // 2, win_rect.bottom + 20 + i * 22))
    ...
    pygame.draw.rect(screen, (0, 0, 0), button_rect.inflate(4, 4), border_radius=12)
...
def draw_game_over(screen):
    ...
    screen.blit(overlay, (0, 0))
    screen_width, screen_height = screen.get_size()
    go_rect = _ui_assets["go_image"].get_rect(center=(screen_width // 2, screen_height // 2 - 80))
    screen.blit(_ui_assets["go_image"], go_rect)
    font_pixel = _ui_assets["font_pixel_small"]
    message_lines = ["you have skill issues, dude~", "better luck next time.."]
    for i, line in enumerate(message_lines):
        text_surface = font_pixel.render(line, True, (255, 255, 255))
